...
base_url = 'https://www.mashreghnews.ir'
db_file_path = './volume/archive.db'
db_connection = sqlite3.connect(db_file_path)
db_cursor = db_connection.cursor()
agency_name = 'MashreghNews'
...
def extract_single_news_information(link, category, agency):
    try:
        ...
        published_date_object = soup.select('div.item-nav div.item-date span')
        ...
        # Get Title
        subtitle_object = soup.select('div.item-title')
        try:
            subtitle = subtitle_object[0].select('h4.subtitle a')[0].get_text().strip()
        except:
            subtitle = ''

        title_main = subtitle_object[0].select('h1.title a')[0].get_text().strip()
        title = subtitle + ' ' + title_main

        # Get Abstract
        abstract_object = soup.select('div.item-summary p.summary')
        try:
        ...
        service_object = soup.select('ol.breadcrumb li')
        ...
        # Get Tags
        tags_list_object = soup.select('div.item-boxes section.tags ul li a')
        tags_list = []
...
def each_day_loop(start_page: int = 0, total_page: int = 50, year: int = 0, month: int = 0, day: int = 0):
    if year == 0 or month == 0 or day == 0:
        ...
        for p in tqdm_bar:
            delay = randint(1, 5)
            try:
                page_link = f'{base_url}/archive?mn={month}&dy={day}&ms=0&pi={p}&yr={year}'
                page = requests.get(page_link, timeout=5)
                soup = BeautifulSoup(page.content, 'html.parser')

                news_list = soup.select('div.itemlist div.items > ul > li figure > a')
